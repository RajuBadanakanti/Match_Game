...
class App extends Component {
  ...
  statusChange = () => {
    ...
    if (time !== 0) {
      ...
      this.setState({isGameOnProcess: false})
    }
  }
  ...
  renderScoreCardSection = () => {
    ...
    return (
      <div className="bg-scorecard-section">
        <img
          src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
          alt="trophy"
          className="trophy-png"
        />
  ...
  onClickThubnail = thumbnailUrl => {
    const {imgUrl} = this.state
    const imgValue = imagesList.filter(
      eachImgData => eachImgData.thumbnailUrl === thumbnailUrl,
    )
  ...
  getFilterImgData = () => {
    ...
    const filterImgData = imagesList.filter(
      eachImgData => eachImgData.category === category,
    )

    return filterImgData
  }

  renderMatchGameSection = () => {
    const filteredImgList = this.getFilterImgData()
    const {category, imgUrl} = this.state

    return (
      <div className="game-images-container">
        <img src={imgUrl} alt="match" className="bg-image-png" />
        <ul className="tabs-ui-container">
          {tabsList.map(eachTab => (
            <li className="tab-list-items" key={eachTab.tabId}>
              <button
                type="button"
                className={`tab-text ${
                  category === eachTab.tabId ? 'tab-text-active' : ''
                }`}
                onClick={() => this.onClickTabBtn(eachTab.tabId)}
              >
                {eachTab.displayText}
              </button>
            </li>
        ...
        <ul className="tumbnails-ul-container">
          {filteredImgList.map(eachImgData => (
            <li
              className="tumbnails-li-container"
              id={eachImgData.id}
              key={eachImgData.id}
            >
              <button
                type="button"
                className="tumbnails-btn"
                onClick={() => this.onClickThubnail(eachImgData.thumbnailUrl)}
              >
                <img
                  src={eachImgData.thumbnailUrl}
                  className="tumbnails-png"
                  alt="thumbnail"
                />
              </button>
            </li>
  ...
  render() {
    const {score, isGameOnProcess, time} = this.state
    return (
      <div className="bg-App-container">
        <navbar className="navbar-container">
          ...
          />
          <ul className="score-ul-div">
            <li className="score-li-items">
